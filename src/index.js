...
const initialQuestion = [
  {
    ...
    choices: ["Create some content", "Export website"]
  }
];
...
inquirer
  ...
  .then(({ pageTitle }) => {
    const { window } = new JSDOM(
      fs.readFileSync(join(__dirname, "template.html")).toString().replace("{{title}}", pageTitle)
    );
    const { document: d } = window;

    const prompt = () => {
      return inquirer.prompt(initialQuestion).then((answers) => {
        if (answers.createOrSave === "Create some content") {
          ...
          fs.writeFileSync(
            join(__dirname, "..", "build", `${pageTitle}.html`),
            "<!DOCTYPE html>" +
            minify(window.document.documentElement.outerHTML, {
              minifyCSS: true,
              minifyJS: true,
              minifyURLs: true,
              removeComments: true,
